    ...
    return (
        <>
            ...
            <form onSubmit={handleRegister} className="flex flex-col px-[30px] mt-[10px] w-full h-full">
                <input onChange={e => setNameUser(e.target.value)} value={nameUser} type="text" placeholder="Имя"
                       className="mt-[5px] text-white placeholder:text-gray-100 border-2 border-gray-100 focus:outline-none text-xl bg-transparent py-[15px] pl-[10px] rounded-2xl"/>
                ...
                <input onChange={e => setPassword(e.target.value)} value={password} type="password" placeholder="Пароль"
                       className="mt-[5px] text-white placeholder:text-gray-100 border-2 border-gray-100 focus:outline-none text-xl bg-transparent py-[15px] pl-[10px] rounded-2xl"/>
                <button className="mt-[10px] bg-gray-100 text-black text-xl py-[15px] rounded-2xl"
                        type="submit">Зарегистрироваться
                </button>
            </form>
            <Link className="text-white underline text-lg w-full text-center mt-[5px] mr-[10px]" href="login">У меня уже
                есть аккаунт</Link>
        </>
    );
};
